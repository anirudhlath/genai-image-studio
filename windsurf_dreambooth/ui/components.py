    ...
    with gr.Tab("Train") as tab:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                imgs = gr.File(file_count="multiple", label="Training Images", file_types=["image"])
                train_steps_slider = gr.Slider(1, 1000, value=150, step=1, label="Training Steps")
                ...
                precision_dropdown = gr.Dropdown(PRECISION_OPTIONS, value="f16", label="Precision")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                estimate_output = gr.Markdown(
                    label="Time Estimate", value="⏱️ Estimated time: calculating..."
                )
